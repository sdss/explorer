...
from bokeh.core.serialization import Deserializer, DeserializationError, Serialized
...
class BokehModel(DOMWidget):
    ...
    def close(self) -> None:
        ...
        # NOTE: ipywidgets calls close() again on gc (__del__), so only
        # detach if we are still in the document callback registry. the
        # registry is deleted outright when the document is destroyed
    ...
    def _sync_model(self, _model: BokehModel, content: dict[str, Any], _buffers: list[Any]) -> None:
        ...
        try:
            ...
            # NOTE: a frontend event can reference a model that was already
            # removed/replaced server side (stale reference). drop it rather
            # than raise an uncaught exception out of the ipywidgets callback
            return
